...
- **白天可读性优化**：注释、操作符、变量、函数等颜色亮度微调，强光下清晰可见，夜间依旧温润。
- **搜索结果高亮分层**：当前匹配项使用主蓝背景 + 边框，其他匹配项更淡，一目了然。
- **异步函数颜色同步**：统一为温润的 `#87cefa`，保留加粗斜体，语义更清晰。
- **更完善的语义修饰符**：覆盖抽象成员、废弃标记、静态成员等。
...
- **Daylight readability optimization**: Fine-tuned brightness of comments, operators, variables, functions, etc. – clear in strong light, yet still gentle at night.
    
- **Search result layering**: Current match now features a primary‑blue background + border; other matches are subtler, making navigation effortless.
    
- **Async function color sync**: Unified to a warm, gentle `#87cefa`, retaining bold italic for clearer semantics.
    
- **Enhanced semantic modifiers**: Now covering abstract members, deprecated markers, static members, and more.
